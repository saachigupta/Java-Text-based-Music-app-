...
public class AudioBook extends AudioContent
{
  ...
  // Play the audiobook by setting the audioFile to the current chapter title (from chapterTitles array list) 
	...
	{
		if (other instanceof AudioBook){ // checks if "other" is an instance of the AudioBook class
		AudioBook otherA = (AudioBook) other; // casts other to a Audiobook
		return (this.author.equals(otherA.getAuthor()) && this.narrator.equals(otherA.getNarrator()));
		}
		return false;
	}
