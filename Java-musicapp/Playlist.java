...
public class Playlist
{
	...
	{
		Playlist otherP = (Playlist) other; // casts object other to an object of Playlst

		return this.getTitle().equals(otherP.getTitle());
	}
	...
	// Additional method created that gets the index of a particular Audicontent in a playlist (Used in deleteSong method in Library)
	public int getContentindex(String title){
		int index = -1;
		for (int i=0; i< contents.size(); i++){
			// goes through the content of the playlist to see if the song exists in the playlist by comparting titles
			if (contents.get(i).getTitle().equalsIgnoreCase(title)){
				index=i;
				break;
			}
		}
