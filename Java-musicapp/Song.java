...
public class Song extends AudioContent implements Comparable<Song> // implement the Comparable interface
{
	...
	{
		if (other instanceof Song){ // checks if "other" is an instance of the Song class
		Song otherS = (Song) other; // casts other to a Song
		return (super.equals(otherS) && composer == otherS.composer && artist == otherS.artist );
		}
		return false;
	}
	...
	{
		return this.getTitle().compareToIgnoreCase(other.getTitle()); //compares irregardless of capitalisation difference
	}
}
